...
For 632.6, I moved the workflow to the Rmd template, added the approximate spatial data, and moved the data publication component of tissue CHN to this workflow. A larger change associated with version 6 is that I moved `annuals 2008`, `soil ph (2010 and 2011)` and `biovolume` to new, indepdendent data sets so that those static components of the projet are no longer integrated with the on-going aspects.

Note that I moved the tissue CHN publishing code here but the new CHN data processing code is in a separate R file. Note also that the publishing component of ICP is still separate owing to the very different nature of that data entity. For this update and, really, available for all future updates, is to blast through the Rmd chunks for everything, and simply paste the dataTable and otherEntity objects that correspond to ICP into the new 632 XML.
...
```{r dataset_details}
projectid <- 632
packageIdent <- 'knb-lter-cap.632.6'
...
# writeAttributes(annuals_biomass) # write data frame attributes to a csv in current dir to edit metadata
# factorsToFrame(annuals_biomass) # write data frame attributes to a csv in current dir to edit metadata
...
# writeAttributes(annuals_composition) # write data frame attributes to a csv in current dir to edit metadata
# factorsToFrame(annuals_composition) # write data frame attributes to a csv in current dir to edit metadata
...
# writeAttributes(fertilizer_application) # write data frame attributes to a csv in current dir to edit metadata
# factorsToFrame(fertilizer_application) # write data frame attributes to a csv in current dir to edit metadata
...
# writeAttributes(stem_growth) # write data frame attributes to a csv in current dir to edit metadata
# factorsToFrame(stem_growth) # write data frame attributes to a csv in current dir to edit metadata
...
# writeAttributes(plant_root_simulator) # write data frame attributes to a csv in current dir to edit metadata
# factorsToFrame(plant_root_simulator) # write data frame attributes to a csv in current dir to edit metadata
...
# factorsToFrame(tissue_chn) # write data frame attributes to a csv in current dir to edit metadata
...
# writeAttributes(annuals_biomass) # write data frame attributes to a csv in current dir to edit metadata
# factorsToFrame(data_element) # write data frame attributes to a csv in current dir to edit metadata

data_element_desc <- "desc"

# create data table based on metadata provided in the companion csv
# use createdataTableFn() if attributes and classes are to be passed directly
data_element_DT <- createDTFF(dfname = data_element,
...
jasonKaye <- addCreator('jaso', 'kaye')
...
dataset <- new("dataset",
               ...
               otherEntity = c(desert_fertilization_sites))
