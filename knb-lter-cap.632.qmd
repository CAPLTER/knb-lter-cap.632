...
# annuals composition and plot characteristics

```{r}
#| label: annuals-key-helpers
#| eval: TRUE

cover_types_year_added <- DBI::dbGetQuery(
  pg,
  "SELECT cover_type, cover_category, year_added FROM urbancndep.cover_types WHERE include IS NOT FALSE;"
)

# warn if any year_added are NULL
n_null_year_added <- DBI::dbGetQuery(
  pg,
  "SELECT COUNT(*) AS n_null FROM urbancndep.cover_types WHERE include IS NOT FALSE AND year_added IS NULL;"
)$n_null
if (!is.null(n_null_year_added) && n_null_year_added > 0) {
  warning(paste(
    "urbancndep.cover_types.year_added has",
    n_null_year_added,
    "NULL values; gating may be inaccurate."
  ))
}

make_rectangular_wide <- function(
    df_long,
    category_keep) {
  df <- df_long |>
    dplyr::mutate(
      cover_category = as.character(cover_category),
  ...
  sampled_df <- df |>
    dplyr::filter(cover_category == "record_keeping", cover_type == "sampled") |>
    dplyr::group_by(dplyr::across(all_of(key_cols))) |>
    dplyr::summarise(
      sampled = safe_max_num(cover_amount),
      sampled_notes = dplyr::first(stats::na.omit(notes)),
      .groups = "drop"
    )

  if (nrow(sampled_df) == 0L) {
    sampled_df <- df |>
      dplyr::distinct(dplyr::across(all_of(key_cols))) |>
      dplyr::mutate(sampled = NA_real_, sampled_notes = NA_character_)
  }

  meta_df <- df |>
    dplyr::group_by(dplyr::across(all_of(key_cols))) |>
    dplyr::summarise(
      sample_date = safe_max_date(sample_date),
      # collector   = dplyr::first(stats::na.omit(collector)),
      .groups = "drop"
    )

  meas_df <- df |>
    dplyr::filter(cover_category == category_keep) |>
    dplyr::mutate(cover_amount = suppressWarnings(as.numeric(cover_amount)))

  # Guardrail: ensure no duplicate cover_type within a key group
  dup_check <- meas_df |>
    dplyr::count(dplyr::across(all_of(key_cols)), cover_type, name = "n") |>
    dplyr::filter(n > 1L)

  if (nrow(dup_check) > 0L) {
    example <- dup_check |> dplyr::slice_head(n = 10)
    msg <- paste0(
      "Duplicate cover_type rows detected within key groups. ",
      "Clean upstream data (DB or pre-processing) so each (",
      paste(key_cols, collapse = ", "), ", cover_type) is unique.\n",
  ...
  # Build full keys × cover_type grid and apply year_added gating with zero-fill
  types_lu <- cover_types_year_added |>
    dplyr::filter(cover_category == category_keep) |>
    dplyr::distinct(cover_type, year_added)

  keys_df <- df |>
    dplyr::distinct(dplyr::across(all_of(key_cols)))

  grid <- tidyr::crossing(keys_df, types_lu)

  meas_filled <- grid |>
    dplyr::left_join(
      meas_df,
      by = c(key_cols, "cover_type")
    ) |>
    dplyr::mutate(
      cover_amount = dplyr::case_when(
        year < year_added ~ NA_real_,
        TRUE ~ dplyr::coalesce(cover_amount, 0)
      )
    )

  meas_idx <- meas_filled |>
    dplyr::arrange(cover_id) |>
    dplyr::group_by(dplyr::across(all_of(key_cols)), cover_type) |>
    dplyr::mutate(idx = dplyr::row_number()) |>
    dplyr::ungroup()

  values_wide <- meas_idx |>
    dplyr::transmute(
      !!!rlang::syms(key_cols),
      name  = paste0(cover_type, "_", idx),
      value = cover_amount
    ) |>
  ...
  out <- df |>
    dplyr::distinct(dplyr::across(all_of(key_cols))) |>
    dplyr::left_join(values_wide, by = key_cols) |>
    dplyr::left_join(sampled_df, by = key_cols) |>
    dplyr::left_join(meta_df, by = key_cols) |>
    dplyr::arrange(year, site_code, plot_id, location_within_plot, subplot)

  # Drop trailing _1 from measurement columns (keys/meta do not match this pattern)
  out <- out |>
    dplyr::rename_with(~ sub("_1$", "", .x), .cols = tidyselect::ends_with("_1"))

  # Final audit: abort if any _1 columns remain (unexpected under invariant)
  remaining <- names(out)[grepl("_1$", names(out))]
  if (length(remaining) > 0L) {
    rlang::abort(paste0(
      "Columns still ending with _1 after normalization: ",
      paste(remaining, collapse = ", "),
      ". Check upstream for duplicates and clean the data."
    ))
  }

  # Reorder columns: fixed keys first, then all remaining alphabetically
  cols_fixed <- c(
    "site_code",
    "year",
    "sample_date",
  ...
  remaining_cols <- sort(setdiff(names(out), cols_fixed))
  out <- out |>
    dplyr::select(dplyr::any_of(cols_fixed), dplyr::all_of(remaining_cols))
...
```{r}
#| label: annuals-composition-wide
#| eval: TRUE
...
SELECT
  ...
  cover_events.patch_type AS location_within_plot,
  cover_events.subplot,
  cover_events.sample_date,
  -- cover_events.collector,
  cover_composition.cover_id,
  cover_types.cover_category,
  cover_types.cover_type,
  cover_composition.cover_amt AS cover_amount,
  cover_composition.notes
FROM
  urbancndep.cover_composition
  JOIN urbancndep.cover_events ON (urbancndep.cover_composition.cover_event_id = urbancndep.cover_events.cover_event_id)
  JOIN urbancndep.cover_types ON (urbancndep.cover_composition.cover_type_id = urbancndep.cover_types.cover_type_id)
  JOIN urbancndep.plots ON (urbancndep.cover_events.plot = urbancndep.plots.id)
  JOIN urbancndep.sites ON (urbancndep.sites.id = urbancndep.plots.site_id)
  JOIN urbancndep.treatments ON (urbancndep.treatments.id = urbancndep.plots.treatment_id)
WHERE
  urbancndep.cover_types.cover_category IN ('annual','record_keeping') AND
  cover_types.include IS NOT FALSE AND
  cover_composition.cover_amt IS NOT NULL AND
  cover_events.year > 2008
ORDER BY
  cover_events.year,
  sites.code,
  plots.id,
  cover_events.patch_type,
  cover_events.subplot,
  cover_types.cover_type,
  cover_composition.cover_id ;
"

annuals_long <- DBI::dbGetQuery(
  conn      = pg,
  statement = sql_annuals
)

annuals_composition <- make_rectangular_wide(
  annuals_long,
  category_keep = "annual"
)
...
```{r}
#| label: annuals-plot-characteristics
#| eval: TRUE
...
FROM
  ...
  JOIN urbancndep.treatments ON (urbancndep.treatments.id = urbancndep.plots.treatment_id)
WHERE
  cover_types.cover_category IN ('plot characteristic','record_keeping') AND
  cover_types.include IS NOT FALSE AND
  cover_composition.include IS NOT FALSE AND
  cover_events.year > 2008
ORDER BY
  cover_events.year,
  sites.code,
...
annuals_plot_chars_long <- DBI::dbGetQuery(
  conn      = pg,
  statement = sql_plotchar
)

annuals_plot_characteristics <- make_rectangular_wide(
  annuals_plot_chars_long,
  category_keep = "plot characteristic"
) |>
  # total_shrub_cover not measured in 2016
  dplyr::mutate(
    total_shrub_cover = dplyr::case_when(
      year == 2016 ~ NA_real_,
      TRUE ~ total_shrub_cover
    )
  )

annuals_plot_characteristics <- annuals_plot_characteristics |>
  dplyr::mutate(
    site_code            = as.factor(site_code),
    plot_id              = as.character(plot_id),
    treatment_code       = as.factor(treatment_code),
...
# try({
#   capeml::write_attributes(annuals_plot_characteristics)
#   capeml::write_factors(annuals_plot_characteristics)
# })

```

```{r}
#| label: annuals-composition-oe
#| eval: FALSE
...
# try({
#   capeml::write_attributes(fertilizer_application)
#   capeml::write_factors(fertilizer_application)
# })
...
# try({
#   capeml::write_attributes(stem_growth)
#   capeml::write_factors(stem_growth)
# })
...
#   capeml::write_factors(desert_fertilization_sampling_sites, overwrite = FALSE)
# })
...
dataset_taxa <- DBI::dbGetQuery(pg, "
    ...
    WHERE
      cover_category = 'annual' AND
      include IS NOT FALSE
    ;
    ") |>
  ...
  dplyr::arrange(cover_type)

capeml::write_taxa_map(
  taxa_df  = dataset_taxa,
  taxa_col = cover_type 
)
...
file.remove(c(list.files(pattern = "^632_")))
