...
make_rectangular_wide <- function(
  ...
  sampled_df <- df |>
    ...
    dplyr::summarise(
      sampled = suppressWarnings(max(as.numeric(cover_amount), na.rm = TRUE)),
  ...
  meta_df <- df |>
    dplyr::group_by(dplyr::across(all_of(key_cols))) |>
    dplyr::summarise(
      sample_date = suppressWarnings(max(as.Date(sample_date), na.rm = TRUE)),
...
WHERE
  ...
  cover_composition.cover_amt IS NOT NULL
ORDER BY
  cover_events.year,
  sites.code,
  plots.id,
...
WHERE
  ...
  cover_types.include IS NOT FALSE AND
  cover_composition.include IS NOT FALSE
ORDER BY
  cover_events.year,
  sites.code,
  plots.id,
