...
```{r upload-to-AWS, eval=TRUE}

source("~/Documents/localSettings/aws.s3")
lapply(list.files(pattern = paste0(632, "_")), capeml::data_to_amz)
```
...
```{r edi-update, eval=TRUE}

capeml::create_package(update = TRUE, environment = "production")
```

## logout

```{r edi-logout, eval=TRUE}
...
lapply(list.files(pattern = paste0(632, "_")), file.remove)
```
